...
- perf(build): the dataset builds into its own chunk. The app-code chunk fell from 1,223 kB to 223 kB (gzip 163 → 68 kB), so a code change no longer invalidates ~92 kB gzipped of unchanged game data — previously one content hash covered both, and they change on completely different schedules
- perf(build): Vite's `json.stringify` emits the dataset as `JSON.parse('…')` rather than a megabyte of object literals, which the engine parses considerably faster

### Notes

- Trimming redundant fields was measured and declined: `RecipeInput.name`, `Recipe.stationName`, and interning drop-source strings together account for 142 kB of 1,049 kB raw, but only ~10 kB gzipped once compression collapses the repetition. Recorded on ROADMAP with the numbers
